...
from lp_utils import ( decode_state, encode_state, FluentState )
...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        #print('add_literal_level', level)

        self.s_levels.append(set())

        for anode in self.a_levels[level-1]:
            for eff in anode.effnodes:
                node = PgNode_s(eff.symbol, eff.is_pos)
                node.parents.add(anode)
        ...
        for i, n1 in enumerate(nodelist[:-1]):
            for n2 in nodelist[i + 1:]:
                if (self.serialize_actions(n1, n2) or
                    ...
                    #print('mutexify ({}, {}) and ({}, {})'.format(n1.action.name, n1.action.args, n2.action.name, n2.action.args))
                    mutexify(n1, n2)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff1 in node_a1.effnodes:
            for eff2 in node_a2.effnodes:
                if eff1.symbol == eff2.symbol and eff1.is_pos != eff2.is_pos:
                    #print("inconsistent_effects_mutex ({}, {}), ({}, {}) --- {} ".format(eff1.symbol, eff1.is_pos, eff2.symbol, eff2.is_pos, True))
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for eff in node_a1.effnodes:
            for pre in node_a2.prenodes:
                if eff.symbol == pre.symbol and eff.is_pos != pre.is_pos:
                    #print('interference_mutex', True)
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a1 in node_s1.parents:
            for a2 in node_s2.parents:
                if a1.is_mutex(a2):
                    mutex = True
                    for s in a1.children:
                        if s == node_s2:
                            mutex = False
                            break
                    for s in a2.children:
                        if s == node_s1:
                            mutex = False
                            break
                    if mutex:
